...
func main() {
	// LoadConfig config
	config := infra.LoadConfig("configs/robot.yaml")
...
func httpServerStop(srv *http.Server) {
	robot.Logger().Info("Shutdown Server ...")
	...
	if err := srv.Shutdown(ctx); err != nil {
		robot.Logger().Fatal("Server Shutdown:" + err.Error())
	}
	robot.Logger().Info("Server exiting")
}
